...
class UserFactory(factory.django.DjangoModelFactory):
    ...
    username = factory.LazyAttribute(lambda _: faker.user_name()) # generates a differnt username evey time with lazy 
    email = factory.LazyAttribute(lambda _: faker.email()) # generates a different email ever ytime with lazy to avoid conflicts 
    password = factory.PostGenerationMethodCall("set_password", "password123") # hashes the password as django stores it hashed 
...
class AppointmentFactory(factory.django.DjangoModelFactory):
    ...
    date_start = timezone.make_aware(datetime(2025, 7, 28, 10, 0, 0) ,timezone.get_current_timezone())
    date_end = timezone.make_aware(datetime(2025, 7, 28, 11, 0, 0), timezone.get_current_timezone())
