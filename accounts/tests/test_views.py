...
from main.models import NotificationPreferences, ProviderProfile, User

from .factories import (CustomerProfileFactory, NotificationPreferencesFactory,
    ...
    def test_post_returns_correct(self, client):
        ...
        response = client.post(
            reverse("signup"),
            {
                ...
                "password1": "salamanca456789234",
                "password2": "salamanca456789234",
                "phone_number": "50555505",
            },
            follow=True,
        )
        ...
        assert response.redirect_chain == [
            (reverse("home"), 302)
        ]  # chain . 302 is what signup does basically because of follow=True
...
class TestUserProfileView:
    ...
    def test_post_change_notifications(self, client, create_user_details):
        ...
        client.force_login(user)
        response = client.post(
            reverse("user_profile"), {"preferences": "none", "changenot": "1"}
        )
    ...
    def test_post_modify_profile(self, client, create_user_details):
        ...
        client.force_login(user)
        response = client.post(
            reverse("user_profile"), {"modify_profile": "1"}, follow=True
        )
        assert response.status_code == 200
    ...
    def test_post_delete_account(self, client, create_user_details):
        ...
        client.force_login(user)
        response = client.post(
            reverse("user_profile"), {"delete_account": "1"}, follow=True
        )
        assert response.status_code == 200
    ...
    def test_post_disconnect_calendar(self, client, create_user_details):
        ...
        provider_profile = ProviderProfileFactory(
            user=user, google_calendar_connected=True
        )
