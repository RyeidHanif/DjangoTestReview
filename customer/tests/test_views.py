...
class TestListProvidersView:
    ...
    def test_unauthenticated_Get(self,mock_get_slots , client , create_user_details):
        ...
        assert response.status_code == 200 
        assert response.redirect_chain == [ ('/accounts/login/?next=/view_providers/', 302) ]
    ...
    def test_post_redirect(self , mock_get_slots ,client , create_user_details):
        ...
        other_provider = ProviderProfileFactory(user=other_user, google_calendar_connected=True)
        client.force_login(user)
        ...
        expected_redirect = reverse("schedule", kwargs={"providerID": other_provider.user.id})
...
class TestScheduleView:
    ...
    def test_unauthentiated_get(self , client, create_user_details ):
        ...
        response =client.get(reverse("schedule", kwargs={"providerID": provider_id}), follow=True)
        assert response.status_code == 200 
    ...
    def test_authenticated_get(self ,mock_available_slots,  client ,create_user_details):
        ...
        client.force_login(user)
        response = client.get(reverse("schedule", kwargs={"providerID":other_provider.id}))
        assert response.status_code == 200 
        template_names = [t.name for  t in response.templates]
    ...
    @patch("main.calendar_client.GoogleCalendarClient.get_available_slots")
    def test_authenticated_post_week(self,mock_calendar_slots , client ,create_user_details):
    ...
    @patch("main.calendar_client.GoogleCalendarClient.get_available_slots")
    def test_authenticated_post_day(self , mock_calendar_slots , client , create_user_details):
        mock_calendar_slots.return_value = []
        user = create_user_details
        client.force_login(user)
    ...
    def test_post_add_appointment(self, mock_calendar_slots , client,  create_user_details):
        user = create_user_details
        mock_calendar_slots.return_value = [
        (timezone.make_aware(datetime(2025, 7, 30, 9, 0, 0), timezone=timezone.get_current_timezone()), timezone.make_aware(datetime(2025, 7, 30, 9, 30, 0), timezone=timezone.get_current_timezone()))
]
        client.force_login(user)
        ...
        assert response.status_code == 200 
        expected_url = reverse("add_appointment", kwargs={"providerUserID": provider.id})
...
class TestAddAppointmentView:
    ...
    def test_dispatch_sets_attributes(self, mock_calendar_client, client):
        url = reverse("add_appointment", kwargs={"providerUserID": self.provider_user.id})
        client.force_login(self.user)
        response = client.get(url)
        assert response.status_code in (200, 302)

    def test_get_normal_mode_no_existing_appointment(self, mock_calendar_client, client):
        url = reverse("add_appointment", kwargs={"providerUserID": self.provider_user.id})
        client.force_login(self.user)
    ...
    def test_get_normal_mode_existing_appointment_redirects(self, mock_calendar_client, client):
        url = reverse("add_appointment", kwargs={"providerUserID": self.provider_user.id})
        client.force_login(self.user)
    ...
    def test_post_normal_mode_confirm_creates_and_redirects(self, mock_calendar_client, client):
        url = reverse("add_appointment", kwargs={"providerUserID": self.provider_user.id})
        client.force_login(self.user)

        with patch("customer.views.AppointmentRecurrenceForm") as mock_form_class, \
             patch("customer.views.create_and_save_appointment") as mock_create_appointment, \
             patch("customer.views.calculate_total_price", return_value=100):
            ...
            response = client.post(url, data={"confirm": "true", "special_requests": "please be on time"})
    ...
    def test_get_reschedule_mode_existing_appointment(self, mock_calendar_client, client):
        url = reverse("add_appointment", kwargs={"providerUserID": self.provider_user.id})
        client.force_login(self.user)
        ...
        mock_appointment = MagicMock(recurrence_frequency="weekly", recurrence_until="2025-12-31")
    ...
    def test_get_reschedule_mode_no_appointment_redirects(self, mock_calendar_client, client):
        url = reverse("add_appointment", kwargs={"providerUserID": self.provider_user.id})
        client.force_login(self.user)
    ...
    def test_post_reschedule_mode_confirm(self, mock_calendar_client, client):
        ...
        with patch("customer.views.AppointmentRecurrenceForm") as mock_form_class, \
             patch("customer.views.Appointment.objects.filter") as mock_filter, \
             patch("customer.views.change_and_save_appointment") as mock_change_appointment:
...
class TestViewAppointmentsView:
    ...
    def test_reschedule_post_valid(self , client, create_user_details ):
        ...
        client.force_login(user)
        response = client.post(reverse("view_appointments") ,{"reschedule": appointment.id}, follow=True)
        assert response.status_code == 200
    ...
    def test_cancel_post_appointment(self, mock_get_service, mock_cancellation, client, create_user_details,status):
        user = create_user_details
        ...
        response = client.post(reverse("view_appointments"), {"cancel": appointment.id}, follow=True)
        
        appointment.refresh_from_db()
        assert response.status_code == 200 or 302
        assert appointment.status == "cancelled"
        mock_cancellation.assert_called_once_with(
            ANY,  # request has to be passed this mocks it 
            user,
            appointment
        )
...
class TestRescheduleView:
    ...
    def test_unauthenticated_get(self ,client ):
        appointment = AppointmentFactory(status="accepted")
        response = client.get(reverse("reschedule", kwargs={"appointment_id": appointment.id}), follow=True)
        assert response.status_code == 200 
    ...
    def test_authenticated_get(self , client, create_user_details):
        ...
        response = client.get(reverse("reschedule", kwargs={"appointment_id": appointment.id}))
        assert response.status_code == 200 
        template_names = [t.name for t in response.templates]
    ...
    def test_authenticated_post(self , client , create_user_details):
        ...
        client.force_login(user)
        appointment = AppointmentFactory(customer=user , status="accepted")
        response = client.post(reverse("reschedule", kwargs= {"appointment_id": appointment.id}), data={"checkschedule": "lalalala"}, follow=True)
        assert response.status_code == 400
        
        redirection= reverse("schedule", kwargs={"providerID": appointment.provider.providerprofile.id})
        assert (redirection, 302) in response.redirect_chain
