...
def EmailConfirmedAppointment(request, customer , provider , date_start , date_end , to_email):
    ...
    if email.send():
        messages.success(request, f"Dear {provider.username}, your email has been sent to the customer . please prepare for the appointment accordingly ")
    else:
...
def EmailDeclinedAppointment(request , customer , provider , reason,  to_email):
    ...
    email = EmailMessage(mail_subject, message, to=[to_email])
    if email.send():
        messages.success(request, f"Dear {provider.username} The email has been sent to the customer . please do not reject appointments unnecessarily or otherwise block that time slot  ")
    else:
        messages.error(request, f'Problem sending confirmation email to {to_email}, check if you typed it correctly.')



def EmailRescheduledAppointment(request , customer , provider ,  old_date_start , old_date_end , new_date_start , new_date_end , to_email):
...
def EmailCancelledAppointment(request , customer , provider ,  to_email):
    ...
    email = EmailMessage(mail_subject, message, to=[to_email])
    if email.send():
        messages.success(request, f"Dear {provider.username} The email has been sent to the customer . please do not cancel appointments unnecessarily  ")
    else:
        messages.error(request, f'Problem sending confirmation email to {to_email}, check if you typed it correctly.')