...
class TestProviderDashboardView:
    ...
    def test_actions_post(self , client , create_user , action , redirect):
        user =create_user
        client.force_login(user)
        response = client.post(reverse("provider_dashboard"), data={action:1}, follow= True)
        assert response.status_code == 200 
...
class TestListAcceptedAppointmentsView:
    ...
    def test_mark_completed_post_valid(self , client , create_user):
        ...
        client.force_login(user)
        response = client.post(reverse("view_my_appointments"), data={"markcompleted": appointment.id})
    ...
    def test_mark_completed_post_invalid(self , client , create_user):
        ...
        appointment = AppointmentFactory(provider=user , status="accepted" , date_start = date_start , date_end = date_end)
        response = client.post(reverse("view_my_appointments"), data={"markcompleted": appointment.id}, follow=True)
        assert response.status_code == 200
    ...
    def test_cancel_appointment_post_valid(self , mock_calendar_client ,mock_cancellation , client , create_user):
        user = create_user
        client.force_login(user)
        ...
        appointment = AppointmentFactory(provider=user , status="accepted", event_id = "some-event-id")
        response = client.post(reverse("view_my_appointments"), data={"cancel": appointment.id})
        ...
        mock_calendar_client_instance.delete_event.assert_called_once_with(user, "some-event-id")
    ...
    @patch("provider.views.cancellation", return_value=0)
    @patch("provider.views.GoogleCalendarClient")
    def test_invalid_cancel_appointment(self , mock_calendar_client_class,mock_cancellation , client, create_user):
        user = create_user
        client.force_login(user)
...
class TestListPendingAppointmentsView:
    ...
    def test_get_pending_appointments_view(self, client, create_user):
        ...
        assert response.status_code == 200
        assert "provider/view_pending_appointments.html" in [t.name for t in response.templates]

    @patch("provider.views.GoogleCalendarClient")
    def test_post_accept_pending_success(self, mock_calendar_client, client, create_user):
        provider, customer = create_user
        client.force_login(provider)
        appointment = AppointmentFactory(provider=provider, customer=customer, status="pending")

        mock_calendar_client.return_value.create_google_calendar_event.return_value = {"id": "mock-event-id"}

        response = client.post(reverse("view_pending_appointments"), data={"accept": appointment.id}, follow=True)
        appointment.refresh_from_db()
    ...
    @patch("provider.views.GoogleCalendarClient")
    def test_post_accept_reschedule_success(self, mock_calendar_client, client, create_user):
        provider, customer = create_user
        client.force_login(provider)
        appointment = AppointmentFactory(provider=provider, customer=customer, status="rescheduled", event_id="abc123")
    ...
    @patch("provider.views.GoogleCalendarClient")
    def test_post_reject_pending_or_reschedule(self, mock_calendar_client, client, create_user):
        provider, customer = create_user
        client.force_login(provider)

        # Pending rejection
        pending_appt = AppointmentFactory(provider=provider, customer=customer, status="pending")
        reschedule_appt = AppointmentFactory(provider=provider, customer=customer, status="rescheduled", event_id="abc123")

        response_pending = client.post(reverse("view_pending_appointments"), data={"reject": pending_appt.id}, follow=True)
        ...
        response_resched = client.post(reverse("view_pending_appointments"), data={"reject": reschedule_appt.id}, follow=True)
    ...
    def test_search_pending_by_name(self, client, create_user):
        ...
        appt_aris = AppointmentFactory(provider=provider, status="pending", customer__first_name="Aris")
        AppointmentFactory(provider=provider, status="pending", customer__first_name="Bob")
